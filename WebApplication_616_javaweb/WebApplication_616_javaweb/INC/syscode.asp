...
sub ShowClass_DownGuide()
	response.write  "&nbsp;<a href='download.asp'>分校</a>&nbsp;&gt;&gt;"
	if BigClassName="" and SmallClassName="" then
		response.write "&nbsp;所有分校"
	else
		if BigClassName<>"" then
...
sub ShowArticleTotal()
	...
	if rsTotal.eof and rsTotal.bof then
		totalPut=0
		response.write "共有 0 个睿源之星"
	else
		totalPut=rsTotal(0)
		response.Write "共有 " & totalPut & " 个睿源之星"
	end if
	rsTotal.close
	set rsTotal=nothing
end sub
...
sub ShowDownTotal()
	...
	if rsTotal.eof and rsTotal.bof then
		totalPut=0
		response.write "共有 0 个分校"
	else
		totalPut=rsTotal(0)
		response.Write "共有 " & totalPut & " 个分校"
	end if
	rsTotal.close
	set rsTotal=nothing
end sub
...
sub ShowArticle(TitleLen)
	...
	if rsArticle.bof and  rsArticle.eof then
		response.Write("<br><li>没有任何睿源之星</li>")
	else
		if currentPage=1 then
...
sub ArticleContent(intTitleLen)
	...
	do while not rsArticle.eof
		...
		strTemp= strTemp & "<table width=100% border=0 cellspacing=2 cellpadding=0>"
                ...
                strTemp= strTemp & "<td width=12% height=12>"
                strTemp= strTemp & "明星年龄：</td>"
                strTemp= strTemp & "<td>" & rsArticle("Price") & "岁</td>"				
				
                strTemp= strTemp & "</tr><tr>"
                strTemp= strTemp & "<td height=12>"
                strTemp= strTemp & "所属类别：</td>"
               ...
               ' strTemp= strTemp & "<td height=12>"
               ' strTemp= strTemp & "睿源之星编号：</td>"
              '  strTemp= strTemp & "<td>" & rsArticle("Product_Id") & "</td>"
              '  strTemp= strTemp & "</tr><tr>"
			   
                strTemp= strTemp & "<td height=12>详细介绍：</td>"
                strTemp= strTemp & "<td>"
...
sub ShowDown(TitleLen)
	...
	if rsDown.bof and  rsDown.eof then
		response.Write("<br><li>没有任何分校</li>")
	else
		if currentPage=1 then
...
sub ShowSearchTerm()
	...
	if keyword<>"" then
	  select case strField
		case "Title"
			response.Write "名称中含有 <font color=red>"&keyword&"</font> 的睿源之星"
		case "Content"
			response.Write "说明含有 <font color=red>"&keyword&"</font> 的睿源之星"
		case else
			response.Write "名称中含有 <font color=red>"&keyword&"</font> 的睿源之星"
	  end select
	else
	  response.write "&nbsp;所有睿源之星"
	end if
end sub
...
sub SearchResultTotal()
	...
	if rsTotal.eof and rsTotal.bof then
		...
		totalPut=rsTotal(0)
		response.Write "共找到 " & totalPut & " 个睿源之星"
	end if
end sub
...
sub ShowSearchResult()
	...
	rsSearch.open sqlSearch,conn,1,1
 	if rsSearch.eof and rsSearch.bof then 
       		response.write "<p align='center'><br><br>没有或没有找到任何睿源之星</p>" 
   	else 
   		if currentPage=1 then 
  ...
  <form method="Get" name="myform" action="search.asp">
    <tr>
      <td height="28"> <select name="Field" size="1">
          <option value="Title" selected>睿源之星名称</option>
          <option value="Content">睿源之星说明</option>
        </select> 
        <%if ShowType=1 then%>
      </td>
    </tr>
...
sub ShowUserLogin()
	...
	If Session("UserName")="" Then
        ...
        strLogin=strLogin & "<tr><td height='25' align='right'><font color=#0F77C0>用户名：</td><td height='25'><input name='UserName' type='text' id='UserName' size='10' maxlength='25'style='font-size: 9pt; border: 1px solid #999999'></td></tr>"
        strLogin=strLogin & "<tr><td height='25' align='right'><font color=#0F77C0>密&nbsp;&nbsp;码：</td><td height='25'><input name='Password' type='password' id='Password' size='10' maxlength='20'style='font-size: 9pt; border: 1px solid #999999'></td></tr>"
        strLogin=strLogin & "<tr align='center'><td height='25' colspan='2'><input name='Login' type='submit' id='Login' value=' 登陆 'style='font-size: 9pt; color: #1576BB; border: 1px solid #1576BB'> <input name='Reset' type='reset' id='Reset' value=' 清除 'style='font-size: 9pt; color: #1576BB; border: 1px solid #1576BB'>"
...
sub ShowUserLogina()
	...
	If Session("UserName")="" Then
        ...
        strLogin=strLogin & "<tr><font color=#111111><td height='25' align='right'>用户名：</td><td height='25'><input name='UserName' type='text' id='UserName' size='10' maxlength='20'></td></tr>"
