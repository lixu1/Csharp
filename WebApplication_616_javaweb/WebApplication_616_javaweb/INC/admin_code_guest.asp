...
sub SaveWriteGuest()
	...
	if CheckCode=""  then
		call Guest_info("<li>留言发送失败！</li><li>请输入验证码！</li>")
		exit sub
	end if
	'if  CheckCode<>CStr(session("CheckCode")) then
	if  CheckCode<>CStr(session("getcode")) then
		call Guest_info("<li>留言发送失败！</li><li>您输入的验证码和系统产生的不一致，请重新输入</li>")
		exit sub
	end if
...
sub Guest_info(info)
	...
	strInfo=strInfo & "<tr align='center' class='tdbg'><td><a href='" & strFileName & "'>【查看留言】</a>" & vbcrlf
